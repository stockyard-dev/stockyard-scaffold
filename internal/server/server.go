...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/templates", s.list)
	s.mux.HandleFunc("POST /api/templates", s.create)
	s.mux.HandleFunc("GET /api/templates/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/templates/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-scaffold", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"templates": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Template{} }
	wj(w, map[string]any{"templates": items, "count": len(items)})
}

func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	var e store.Template
