...
async function load(){var r=await fetch(A+'/templates').then(function(r){return r.json()});items=r.templates||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)||(i.language||'').toLowerCase().includes(q)||(i.use_count||'').toLowerCase().includes(q)});
...
if(i.language)h+='<span>'+esc(i.language)+'</span>';
if(i.use_count)h+='<span>'+esc(i.use_count)+'</span>';
if(i.path)h+='<span>'+esc(i.path)+'</span>';
if(i.content)h+='<span>'+esc(i.content)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/templates/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",description:"",language:"",use_count:"",path:"",content:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SCAFFOLD</h2>';
...
h+='<div class="fr"><label>Language</label><input id="f-language" value="'+esc(i.language)+'"></div>';
h+='<div class="fr"><label>Use Count</label><input id="f-use-count" value="'+esc(i.use_count)+'"></div>';
h+='<div class="fr"><label>Path</label><input id="f-path" value="'+esc(i.path)+'"></div>';
h+='<div class="fr"><label>Content</label><textarea id="f-content" rows="3">'+esc(i.content)+'</textarea></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),description:document.getElementById('f-description').value.trim(),language:document.getElementById('f-language').value.trim(),use_count:document.getElementById('f-use-count').value.trim(),path:document.getElementById('f-path').value.trim(),content:document.getElementById('f-content').value.trim()};
if(editId){await fetch(A+'/templates/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/templates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
